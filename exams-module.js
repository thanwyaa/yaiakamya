...
async function openLessonExam(lessonId) {
    ...
    // التحقق من إكمال الامتحان (مرة واحدة فقط)
...
// ===== عرض واجهة الامتحان مع منع النسخ والغش =====
...
// ===== دوال الكويزات (مختصرة ولكن بنفس الحماية) =====
// ... (نفس المنطق مع تعديلات طفيفة)
...
function bindExamFunctions() {
    if (window.APP) {
        ...
        window.APP.submitQuiz = submitQuiz; // سيتم ربطها لاحقاً
        window.APP.exitExam = exitExam;
        window.APP.navigateExamQuestion = navigateExamQuestion;
        window.APP.navigateQuizQuestion = navigateQuizQuestion; // سيتم ربطها
