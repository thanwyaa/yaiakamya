...
function loadCourses() {
    if (window.cache && window.cache.courses && window.cache.courses.length > 0 && isCacheValid()) {
        window.allCourses = window.cache.courses;
        renderCourses(window.allCourses);
        return;
    }

    window.database.ref('courses').once('value', (snapshot) => {
        window.allCourses = [];
        ...
        if (window.cache) window.cache.courses = window.allCourses;
        updateCache();
        renderCourses(window.allCourses);
...
function loadLessons() {
    if (window.cache && window.cache.lessons && window.cache.lessons.length > 0 && isCacheValid()) {
        window.allLessons = window.cache.lessons;
        return;
    }

    window.database.ref('lessons').once('value', (snapshot) => {
        window.allLessons = [];
        snapshot.forEach((child) => {
            const data = child.val();
            window.allLessons.push({ id: child.key, ...data });
        });
        if (window.cache) window.cache.lessons = window.allLessons;
        updateCache();
    });
}

function loadExams() {
    if (window.cache && window.cache.exams && window.cache.exams.length > 0 && isCacheValid()) {
    ...
    window.database.ref('exams').once('value', (snapshot) => {
        ...
        if (window.cache) window.cache.exams = window.allExams;
        updateCache();
    });
}

function loadQuizzes() {
    if (window.cache && window.cache.quizzes && window.cache.quizzes.length > 0 && isCacheValid()) {
    ...
    window.database.ref('quizzes').once('value', (snapshot) => {
        ...
        if (window.cache) window.cache.quizzes = window.allQuizzes;
        updateCache();
    });
}

function loadUserSubscriptions(uid) {
    if (window.cache && window.cache.subscriptions && window.cache.subscriptions.length > 0 && isCacheValid()) {
    ...
    window.database.ref('courseSubscriptions').orderByChild('userId').equalTo(uid).once('value', (snapshot) => {
        ...
        if (window.cache) window.cache.subscriptions = window.userSubscriptions;
        updateCache();
        renderCourses(window.allCourses);
    });
}
...
function loadUserProgress(uid) {
    if (window.cache && window.cache.progress && Object.keys(window.cache.progress).length > 0 && isCacheValid()) {
...
async function subscribeToCourse(courseId) {
    ...
    try {
        ...
        if (course.isFree === false) {
            // Show premium page for paid courses
