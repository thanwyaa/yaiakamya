...
function openExamSecurityModal(examId, callback, type = 'exam') {
    ...
    if (titleEl) {
        titleEl.textContent = type === 'quiz' ? '🧪 قبل بدء الكويز' : '📝 قبل بدء الاختبار';
    }
    window.pendingExamCallback = callback;
...
function startExamAfterPledge() {
    if (window.pendingExamCallback) {
        const callback = window.pendingExamCallback;
        const examIdToSave = window.currentExamId;
        window.pendingExamCallback = null;
        window.currentExamId = null;
        document.getElementById('examSecurityModal').classList.remove('open');
        if (window.currentUser && examIdToSave) {
            window.database.ref(`users/${window.currentUser.uid}/examPledges/${examIdToSave}`).set({
                agreed: true,
                agreedAt: new Date().toISOString()
            }).catch(err => console.error('Error saving pledge:', err));
        }
        if (typeof callback === 'function') {
            callback();
        } else {
            showToast('حدث خطأ داخلي، يرجى المحاولة مرة أخرى.', 'error');
        }
    }
}
...
function renderExamUI(exam, startTime, duration) {
    ...
    const validStartTime = typeof startTime === 'number' ? startTime : parseInt(startTime) || Date.now();
    let timeLeft = Math.max(0, (duration * 60) - Math.floor((Date.now() - validStartTime) / 1000));
    ...
    window.examStartTime = validStartTime;
    window.currentExamId = exam.id;
    window.examAnswers[exam.id] = {};
    window.isExamActive = true;
    window.examSubmitted = false;
    ...
    if (duration > 0 && timeLeft > 0) {
        window.examTimer = setInterval(() => {
            const elapsed = Math.floor((Date.now() - validStartTime) / 1000);
            timeLeft = Math.max(0, (duration * 60) - elapsed);
            ...
            if (timeLeft <= 0 && !window.examSubmitted) {
                clearInterval(window.examTimer);
                window.examTimer = null;
                window.isExamActive = false;
                submitExam(exam.id, exam.atomsReward || 10, true);
    ...
    window.selectExamAnswer = function(id, qIdx, oIdx, correct) {
        if (!window.isExamActive || window.examSubmitted) return;
        if (!window.examAnswers[id]) window.examAnswers[id] = {};
        ...
        window.examAnswers[id][qIdx] = { selected: oIdx, correct: correct };
    ...
    const renderQuestion = (index) => {
        ...
        if (progress) progress.textContent = `${index + 1} / ${questions.length}`;
        ...
        <div style="max-width:800px;margin:0 auto;padding:16px;">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;justify-content:space-between;">
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    <button class="btn-outline btn-sm" id="prevQuestion" onclick="APP.navigateExamQuestion(-1)" style="display:${questions.length > 1 ? 'inline-flex' : 'none'}">
                        <i class="fas fa-chevron-right"></i> السابق
                    </button>
                    <button class="btn-outline btn-sm" id="nextQuestion" onclick="APP.navigateExamQuestion(1)">
                        التالي <i class="fas fa-chevron-left"></i>
                    </button>
                </div>
                <button class="btn-primary" id="submitExamBtn" onclick="APP.submitExam('${exam.id}', ${exam.atomsReward || 10})" style="display:inline-flex;">
                    <i class="fas fa-check"></i> تسليم الامتحان
                </button>
            </div>
        </div>
    `;
...
function navigateExamQuestion(direction) {
    ...
    const prevBtn = document.getElementById('prevQuestion');
    const nextBtn = document.getElementById('nextQuestion');
    if (prevBtn) prevBtn.style.display = next > 0 ? 'inline-flex' : 'none';
    if (nextBtn) nextBtn.style.display = next < cards.length - 1 ? 'inline-flex' : 'none';
}
...
async function submitExam(id, maxAtoms, autoSubmit = false) {
    ...
    const progressKey = 'exam_' + (window.currentExamData?.lessonId || id);
    ...
    if (window.examSubmitted) {
        showToast('تم تسليم الامتحان بالفعل', 'info');
        return;
    }

    const answers = window.examAnswers[id] || {};
    const totalQuestions = window.currentExamData?.questions?.length || 0;
    ...
    try {
        ...
        if (!snapshot.exists()) { 
            showToast('المحتوى غير موجود', 'error'); 
            return; 
        }
        ...
        await resultRef.set({
            ...
            wrongAnswers: wrongQuestions.length,
            timeSpent: timeSpent,
            atomsEarned: earnedAtoms,
            completedAt: new Date().toISOString(),
            lessonId: data.lessonId || id,
            examId: id
        });

        if (earnedAtoms > 0) {
            const atomRef = window.database.ref('users/' + window.currentUser.uid + '/atoms');
            await atomRef.transaction((current) => {
                return (current || 0) + earnedAtoms;
            });
        ...
        if (score === 100) {
            ...
            await perfectRef.transaction((current) => {
                return (current || 0) + 1;
            });
        ...
        if (wrongQuestions.length > 0) {
            ...
            wrongQuestions.forEach(wq => {
                ...
                if (!exists) {
                    ...
                    if (idx !== -1) {
                        errors[idx].attempts = (errors[idx].attempts || 1) + 1;
                    }
                }
            });
        ...
        await window.database.ref(`users/${window.currentUser.uid}/examSessions/${id}`).remove();

        window.userCourseProgress[progressKey] = {
            completed: true,
            score: score,
            atomsAwarded: earnedAtoms,
            completedAt: new Date().toISOString(),
        ...
        if (typeof addNotification === 'function') {
            await addNotification(window.currentUser.uid, '📝 تم إكمال امتحان!', `لقد أكملت امتحان "${data.title || 'الامتحان'}" وحصلت على ${earnedAtoms} ذرة.`, '📝');
        }
...
function showExamResults(examId, score, correct, wrong, results, atomsEarned) {
    ...
    const timeSpent = Math.floor((Date.now() - window.examStartTime) / 60);
    ...
    if (score === 100) {
        message = '🌟 ممتاز جداً، إجابة كاملة، استمر بهذا المستوى.';
        emoji = '🏆';
        gradeColor = 'var(--gold)';
        detailedMessage = 'رائع جداً، إجابة كاملة. استمر بهذا المستوى المتميز!';
    } else if (score >= 90) {
        message = '🎉 أداء رائع جداً، اقتربت من العلامة الكاملة.';
        emoji = '🌟';
        gradeColor = 'var(--gold)';
        detailedMessage = 'أداء رائع جداً، أنت على بعد خطوة من الكمال!';
    } else if (score >= 80) {
        message = '👏 ممتاز، استمر وستصل إلى الدرجة النهائية.';
        emoji = '⭐';
        gradeColor = 'var(--success)';
        detailedMessage = 'ممتاز، استمر في التحسين للوصول إلى الكمال!';
    } else if (score >= 70) {
        message = '👍 جيد جداً، راجع بعض النقاط البسيطة.';
        emoji = '💪';
        gradeColor = 'var(--primary)';
        detailedMessage = 'جيد جداً، راجع بعض النقاط البسيطة وحسّن أدائك!';
    } else if (score >= 60) {
        message = '📚 جيد، لكن تحتاج إلى مراجعة بعض الدروس.';
        emoji = '📖';
        gradeColor = 'var(--warning)';
        detailedMessage = 'جيد، لكن تحتاج إلى مراجعة بعض الدروس لتحسين الفهم.';
    } else if (score >= 50) {
        message = '⚠️ مقبول، ننصح بإعادة مراجعة الدرس قبل المتابعة.';
        emoji = '⚠️';
        gradeColor = '#FFA726';
        detailedMessage = 'مقبول، ننصح بإعادة مراجعة الدرس قبل المتابعة.';
    } else {
        message = '❌ تحتاج إلى مراجعة الدرس بالكامل ثم إعادة التدريب.';
        emoji = '🔴';
    ...
    main.innerHTML = `
        <div style="max-width:900px;margin:0 auto;padding:20px;">
            ...
            <div class="card" style="padding:28px;margin-top:12px;text-align:center;border:3px solid ${gradeColor};">
                ...
                <h2 style="font-family:'Lalezar',cursive;font-size:2.2rem;color:${gradeColor};">
                    ${score >= 50 ? '✅ نجاح' : '❌ يحتاج مراجعة'}
                </h2>
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:16px;margin-top:20px;">
                    <div style="background:var(--bg);padding:16px;border-radius:var(--radius);">
                        ...
                        <div style="font-size:0.85rem;color:var(--text2);">الدرجة</div>
                    </div>
                    <div style="background:var(--bg);padding:16px;border-radius:var(--radius);border:2px solid var(--success);">
                        <div style="font-size:2rem;font-weight:700;color:var(--success);">${correct}</div>
                        <div style="font-size:0.85rem;color:var(--text2);">✅ إجابة صحيحة</div>
                    </div>
                    <div style="background:var(--bg);padding:16px;border-radius:var(--radius);border:2px solid var(--danger);">
                        <div style="font-size:2rem;font-weight:700;color:var(--danger);">${wrong}</div>
                        <div style="font-size:0.85rem;color:var(--text2);">❌ إجابة خاطئة</div>
                    </div>
                    <div style="background:var(--bg);padding:16px;border-radius:var(--radius);">
                        <div style="font-size:2rem;font-weight:700;color:var(--warning);">${timeSpent} د</div>
                        <div style="font-size:0.85rem;color:var(--text2);">⏱ الوقت المستغرق</div>
                    </div>
                    ${atomsEarned > 0 ? `
                        <div style="background:var(--bg);padding:16px;border-radius:var(--radius);border:2px solid var(--gold);">
                            <div style="font-size:2rem;font-weight:700;color:var(--gold);">+${atomsEarned}</div>
                            <div style="font-size:0.85rem;color:var(--text2);">⚛️ ذرات مكتسبة</div>
                        </div>
                    ` : ''}
                </div>
            ...
            ${wrongResults.length > 0 ? `
                <h3 style="font-weight:700;font-size:1.2rem;color:var(--danger);margin:20px 0 12px;">
                    ❌ الأسئلة الخاطئة (${wrongResults.length})
                </h3>
...
function showQuizUI(quiz, isSecure = false) {
    ...
    if (questions.length === 0) {
        showToast('لا توجد أسئلة في هذا الكويز', 'warning');
        return;
    }

    if (window.userCourseProgress['quiz_' + (quiz.lessonId || quiz.id)]?.completed) {
        showCompletedModal();
        return;
    }
    ...
    window.selectQuizAnswer = function(id, qIdx, oIdx, correct) {
        if (!window.isExamActive || window.examSubmitted) return;
        if (!window.quizAnswers[id]) window.quizAnswers[id] = {};
        ...
        window.quizAnswers[id][qIdx] = { selected: oIdx, correct: correct };
        answeredQuestions.add(qIdx);
    ...
    main.innerHTML = `
        <div style="max-width:800px;margin:0 auto;padding:16px;">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px;justify-content:space-between;">
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    <button class="btn-outline btn-sm" id="quizPrev" onclick="APP.navigateQuizQuestion(-1)" style="display:${questions.length > 1 ? 'inline-flex' : 'none'}">
                        <i class="fas fa-chevron-right"></i> السابق
                    </button>
                    <button class="btn-outline btn-sm" id="quizNext" onclick="APP.navigateQuizQuestion(1)">
                        التالي <i class="fas fa-chevron-left"></i>
                    </button>
                </div>
                <button class="btn-primary" id="submitQuizBtn" onclick="APP.submitQuiz('${quiz.id}', ${quiz.atomsReward || 5})" style="display:inline-flex;">
                    <i class="fas fa-check"></i> تسليم الكويز
                </button>
            </div>
        </div>
    `;
...
async function submitQuiz(id, maxAtoms, autoSubmit = false) {
    ...
    const progressKey = 'quiz_' + (window.currentQuizData?.lessonId || id);
    ...
    if (window.examSubmitted) {
        showToast('تم تسليم الكويز بالفعل', 'info');
        return;
    }

    const answers = window.quizAnswers[id] || {};
    const totalQuestions = window.currentQuizData?.questions?.length || 0;
    ...
    try {
        ...
        if (typeof addNotification === 'function') {
            await addNotification(window.currentUser.uid, '🧪 تم إكمال كويز!', `لقد أكملت كويز "${data.title || 'الكويز'}" وحصلت على ${earnedAtoms} ذرة.`, '🧪');
        }
...
async function openLessonExam(lessonId) {
    ...
    if (await isAssessmentCompleted(lessonId, 'exam')) {
        showCompletedModal();
        return;
    }
    
    try {
        const snapshot = await window.database.ref('exams')
            .orderByChild('lessonId')
            .equalTo(lessonId)
            .once('value');
        
        if (!snapshot.exists()) {
            showToast('⚠️ لا يوجد امتحان لهذه الحصة', 'warning');
            return;
        }
        
        let exam = null;
        snapshot.forEach(child => {
            exam = { id: child.key, ...child.val() };
        });
        
        if (!exam) {
            showToast('⚠️ حدث خطأ في تحميل الامتحان', 'error');
            return;
        }
        ...
        console.error('openLessonExam error:', err);
...
async function openLessonQuiz(lessonId) {
    ...
    if (await isAssessmentCompleted(lessonId, 'quiz')) {
        showCompletedModal();
        return;
    }
    
    try {
        const snapshot = await window.database.ref('quizzes')
            .orderByChild('lessonId')
            .equalTo(lessonId)
            .once('value');
        
        if (!snapshot.exists()) {
            showToast('⚠️ لا يوجد كويز لهذه الحصة', 'warning');
            return;
        }
        
        let quiz = null;
        snapshot.forEach(child => {
            quiz = { id: child.key, ...child.val() };
        });
        
        if (!quiz) {
            showToast('⚠️ حدث خطأ في تحميل الكويز', 'error');
            return;
        }
        ...
        console.error('openLessonQuiz error:', err);
...
async function isAssessmentCompleted(lessonId, type) {
    ...
    if (window.userCourseProgress[key]?.completed) return true;
